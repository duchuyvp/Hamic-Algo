# Hamic-Algo

CSES

## Introductory Problems

-   [Weird Algorithm](Weird%20Algorithm.cpp)
-   [Missing Number](Missing%20Number.cpp)
-   [Repetitions](Repetitions.cpp)
-   [Increasing Array](Increasing%20Array.cpp)
-   [Permutations](Permutations.cpp)
-   [Number Spiral](Number%20Spiral.cpp)
-   [Two Knights](Two%20Knights.cpp)
-   [Two Sets](Two%20Sets.cpp)
-   [Bit Strings](Bit%20Strings.cpp)
-   [Trailing Zeros](Trailing%20Zeros.cpp)
-   [Coin Piles](Coin%20Piles.cpp)
-   [Palindrome Reorder](Palindrome%20Reorder.cpp)
-   [Creating Strings I](Creating%20Strings%20I.cpp)
-   [Apple Division](Apple%20Division.cpp)
-   [Tower of Hanoi](Tower%20of%20Hanoi.cpp)
-   [Gray Code](Gray%20Code.cpp)

## Sorting and Searching

-   [Room Allocation](Room%20Allocation.cpp)
-   [Reading Books](Reading%20Books.cpp)
-   [Subarray Sums I](Subarray%20Sums%20I.cpp)
-   [Subarray Divisibility](Subarray%20Divisibility.cpp)
